...
const round=new URL(req.url).searchParams.get('round');const {room,person,round:r}=await current(req,round);if(r.type!=='photo'&&!r.photo)throw new AppError('Esta rodada não recebe fotos.');
