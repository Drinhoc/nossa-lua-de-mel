...
const res=await fetch(origin+'/api/export',{headers:auth});if(!res.ok){console.error('Export falhou:',res.status,(await res.text()).slice(0,300));process.exit(1);}
const exp=await res.json();
const photos=new Map();
for(const p of exp.photos){const r=await fetch(origin+p.download,{headers:auth});if(!r.ok)throw new Error(`Foto ${p.file}: HTTP ${r.status}`);photos.set(p.id,new Uint8Array(await r.arrayBuffer()));}
